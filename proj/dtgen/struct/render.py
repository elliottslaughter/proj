    ...
    return list(
        set(
            itertools.chain.from_iterable(
                header_includes_for_feature(feature) for feature in spec.features
            )
        )
    )
...
def impl_includes_for_features(spec: StructSpec) -> Sequence[IncludeSpec]:
    return list(
        set(
            itertools.chain.from_iterable(
                impl_includes_for_feature(feature) for feature in spec.features
            )
        )
    )
...
def render_struct_block(spec: StructSpec, f: TextIO) -> Iterator[None]:
    with render_utils.render_struct_block(
        name=spec.name, template_params=spec.template_params, f=f
    ):
        yield


def render_template_args(params: Sequence[str], f: TextIO) -> None:
    f.write("".join(["<", ", ".join(params), ">"]))


def render_template_app(
    spec: StructSpec, f: TextIO, with_namespace: bool = False
) -> None:
    if with_namespace:
...
def render_struct_impl_scope(
    spec: StructSpec, f: TextIO, return_type: Optional[str] = None
) -> None:
    if len(spec.template_params) > 0:
...
def render_constructor_decl(spec: StructSpec, f: TextIO) -> None:
    render_utils.render_function_declaration(
        ...
        return_type=None,
        name=spec.name,
        args=[f"{field.type_} const &{field.name}" for field in spec.fields],
        is_const=False,
        f=f,
    )
...
def render_constructor_impl(spec: StructSpec, f: TextIO) -> None:
    with render_utils.render_function_definition(
        ...
        initializer_list=[
            ...
            if not field.indirect
            else f"{field.name}_ptr(std::make_shared<{field.type_}>({field.name}))"
            for field in spec.fields
        ],
        f=f,
    ):
