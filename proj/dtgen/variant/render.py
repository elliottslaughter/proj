...
def source_includes_for_features(spec: VariantSpec) -> Sequence[IncludeSpec]:
    return list(
        set(
            itertools.chain.from_iterable(
                source_includes_for_feature(feature) for feature in spec.features
            )
        )
    )
        ...
        with render_switch_block(cond="this->index()", f=f):
            for idx, value in enumerate(spec.values):
                with render_case(cond=str(idx), include_break=False, f=f):
                    with sline(f):
                        f.write(
                            f"{return_type_var} result = v(this->get<{value.type_}>())"
                        )
                    with sline(f):
            ...
            with render_default_case(include_break=False, f=f):
                with sline(f):
                    f.write(
                        f'throw std::runtime_error(fmt::format("Unknown index {{}} for type {spec.name}", this->index()))'
                    )
