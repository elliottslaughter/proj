...
#include <string>
#include "person/variant_indirect.dtg.hh"
#include <fmt/format.h>

using ::FlexFlow::VariantIndirect;
using ::nlohmann::json;

TEST_SUITE(TP_TEST_SUITE) {
  TEST_CASE("VariantIndirect") {
    int i = 5;

    SUBCASE("brace construction (int)") {
      auto x = VariantIndirect{i};
      CHECK(x.is_num());
      CHECK(!x.is_nested());
      CHECK(x.require_num() == i);
    }

    SUBCASE("brace construction (nested)") {
      auto x = VariantIndirect{VariantIndirect{i}};
      CHECK(x.is_nested());
      CHECK(!x.is_num());
      CHECK(x.require_nested().is_num());
      CHECK(!x.require_nested().is_nested());
      CHECK(x.require_nested().require_num() == i);
    }
