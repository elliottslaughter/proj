...
def _get_repo_rel_path_for_file(file: File, extension_config: ExtensionConfig) -> RepoRelPath:
    return RepoRelPath(get_fullpath(file, extension_config).relative_to(file.repo.path), file.repo)
...
def _get_fullpath_for_file(file: File, extension_config: ExtensionConfig) -> PurePath:
    ...
    group_dir = file.group.group_path
    ...
    if file.role == RoleInGroup.PUBLIC_HEADER:
        ...
        rel = PurePath('include') / library_name / group_dir / (group_name + '.dtg' + source_extension)
    else:
        raise ValueError()

    return _get_fullpath_for_library_rel_path(LibraryRelPath(rel, library))
