...
def test_run_dtgen_for_bin() -> None:
    ...
    file_tree = EmulatedFileTreeWithMtime.from_lists(
        ...
        files=[
            (
                'bin/example/include/example/integer.dtg.toml',
                1,
                toml_contents,
            ),
        ],
    ...
    correct = EmulatedPathTree.from_lists(
        files=[
            'bin/example/include/example/integer.dtg.toml',
            'bin/example/include/example/integer.dtg.h',
            'bin/example/src/example/integer.dtg.cc',
        ],
        dirs=[],
    )
...
size_t hash<FlexFlow::Color>::operator()(FlexFlow::Color x) const {
...
std::string format_as(Color x) {
  ::nlohmann::json j = x;
  return j.dump();
}

std::ostream &operator<<(std::ostream &s, Color x) {
...
void to_json(::nlohmann::json &j, Color x) {
  switch (x) {
    case Color::RED:
      j = "RED";
      break;
...
Gen<FlexFlow::Color> Arbitrary<FlexFlow::Color>::arbitrary() {
  return gen::element<FlexFlow::Color>(
    FlexFlow::Color::RED,
    FlexFlow::Color::BLUE,
    FlexFlow::Color::YELLOW);
}
...
def test_generate_source_contents() -> None:
    ...
    assert cpp_tokenize(result) == cpp_tokenize(correct)
