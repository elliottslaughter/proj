...
def test_tee_output_to_stdout() -> None:
    stdout = io.BytesIO()
    stderr = io.BytesIO()
    (stdout2_val, stderr2_val) = tee_output(['echo', 'hello world'], stdout=stdout, stderr=stderr)
    ...
    assert stdout1_val == b'hello world\n'
    assert stderr1_val == b''

def test_tee_output_no_trailing_newline() -> None:
    stdout = io.BytesIO()
    stderr = io.BytesIO()
    (stdout2_val, stderr2_val) = tee_output(['echo', '-n', 'hello world'], stdout=stdout, stderr=stderr)
    stdout1_val = stdout.getvalue()
    stderr1_val = stderr.getvalue()
...
def test_tee_output_to_stderr() -> None:
    ...
    correct_stderr_val = b'error world\n'
    correct_stdout_val = b'okay world\n'

    assert (stderr1_val, stdout1_val) == (correct_stderr_val, correct_stdout_val)

def test_tee_output_only_to_stderr() -> None:
    stdout = io.BytesIO()
    stderr = io.BytesIO()
    try:
        (stdout2_val, stderr2_val) = tee_output('echo -n "error world" 1>&2', stdout=stdout, stderr=stderr, shell=True)
    except CalledProcessError as e:
        stdout2_val = e.stdout
        stderr2_val = e.stderr
    stdout1_val = stdout.getvalue()
    ...
    correct_stderr_val = b'error world'
    correct_stdout_val = b''

    assert (stderr1_val, stdout1_val) == (correct_stderr_val, correct_stdout_val)

def test_tee_output_output() -> None:
    stdout = io.BytesIO()
    stderr = io.BytesIO()
    (stdout2_val, stderr2_val) = tee_output('echo "error world" 1>&2 && echo "okay world"', stdout=stdout, stderr=stderr, shell=True)
    stdout1_val = stdout.getvalue()
